...
const ChartLegendContent = React.forwardRef<HTMLDivElement, ChartLegendProps>(
  ({ className, nameKey, payload, verticalAlign = "middle" }, ref) => {
    const { config } = useChart();
    ...
    return (
      <div
        ref={ref}
        className={cn("flex items-center justify-center gap-1.5", verticalAlign === "top" && "items-start", className)}
      >
        {payload.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xs text-muted-foreground">{itemConfig?.label || item.name}</span>
            </div>
          );
        })}
      </div>
