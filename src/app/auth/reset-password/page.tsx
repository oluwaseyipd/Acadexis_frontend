...
import { z } from "zod";
import { ArrowLeftIcon, Eye, EyeOff, ShieldCogIcon, CheckCircle2 } from "lucide-react";
...
function ResetPasswordForm() {
  ...
  const onSubmit = async (data: PasswordFormData) => {
    if (!token) {
      setServerError("Reset token is missing from the link. Please request a new link.");
      return;
    }
    ...
    try {
      await apiService.auth.resetPassword(token, data.password);
      setSuccessMessage("Your password has been reset successfully! Redirecting you to login...");
      setTimeout(() => {
          ...
          <div className="rounded-md bg-gray-100">
            <div className="flex">
              ...
              <div className="flex items-start gap-4 px-4 py-3 text-black text-sm rounded-b-lg">
                <ShieldCogIcon size={45} className="text-green-500 text-[2.5rem]" />
      ...
      <div className="flex items-center justify-center px-4 py-10">
        ...
        <Suspense fallback={
          <div className="flex flex-col items-center gap-3">
            <Spinner />
            <p className="text-sm text-gray-500 font-medium">Loading panel...</p>
          </div>
        }>
          <ResetPasswordForm />
        </Suspense>
